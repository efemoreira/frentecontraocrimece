...
export default function About() {
	...
	return (
		<section id="sobre" className="relative overflow-hidden py-20 md:py-32 bg-white">
			{/* Background Fixo */}
			...
			<div className="container relative z-10 px-4 sm:px-6 md:px-8">
				...
				>
					<h2 className="text-2xl sm:text-3xl md:text-5xl font-bold mb-3 md:mb-4 text-yellow-500">
						Audiência Pública na ALECE
					</h2>
				...
				<div className="flex flex-col md:grid md:grid-cols-5 gap-4 md:gap-8 max-w-5xl mx-auto">
					...
					<motion.div
						...
						viewport={{ once: true }}
						className="md:col-span-2 bg-yellow-50/80 backdrop-blur-sm rounded-lg p-3 md:p-4"
					>
							...
							{tabs.map((tab, index) => (
								<React.Fragment key={tab.id}>
									<button
										...
										className={`relative py-3 px-4 rounded-md text-center md:text-left transition-all duration-200 text-sm md:text-lg font-medium ${
											activeTab === tab.id
												? 'bg-yellow-400/30 text-black'
												: 'text-black/70 hover:bg-yellow-200/40'
										}`}
									>
										{tab.label}
										{activeTab === tab.id && (
											<span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-1/2 bg-yellow-500 rounded-r-full"></span>
										)}
									</button>
									{index < tabs.length - 1 && (
										<div className="hidden md:block h-px bg-yellow-200 my-2 mx-3"></div>
									)}
								</React.Fragment>
							))}
						</div>
							...
							>
								...
								<div className="mt-6 md:mt-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
									...
									<motion.a
										...
										whileTap={{ scale: 0.95 }}
										href="#inscricao"
										className="btn btn-primary inline-block text-center py-3 px-6 bg-yellow-400 text-black border-none hover:bg-yellow-300"
									>
										Quero Participar
									</motion.a>
								</div>
